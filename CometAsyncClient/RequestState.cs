...
namespace CometAsyncClient
{
    public class CometRequest{
        ...
        public void BeginWaitRequest<T>(string url, string[] parameters, string[] values)
        {
            if (this.requestState == null)
            {
                ...
                if (parameters != null && values != null && parameters.Length == values.Length)
                {
                    ...
                    {
                        //  append for values
                        postData.AppendFormat("&{0}={1}", parameters[i], values[i]);
                    }

                    //  and now create a postdata array
                    byte[] content = ASCIIEncoding.UTF8.GetBytes(postData.ToString());
        ...
        public void BeginWaitRequest<T>(string url, string data)
        {
            if (this.requestState == null)
            {
                //  ok, create the request object that is going to perform this
                ...
                if (string.IsNullOrEmpty(data))
                {
                    ...
                    //  and now create a postdata array
                    byte[] content = ASCIIEncoding.UTF8.GetBytes(data);
                    //  and the content length
                    request.ContentLength = content.Length;
